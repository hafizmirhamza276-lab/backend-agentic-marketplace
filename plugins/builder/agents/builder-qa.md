...
**Bug-fix mode (when `.claude/builder/BUG.md` exists).** The deterministic `regression-gate.sh` owns the red→green proof; your job is to corroborate and widen it. Fold the repro result (must be GREEN post-fix) and the characterization/linked results into the coverage map, and probe the broader regression surface around the fix's blast radius (callers from MEMORY.md). Flag any symptom-only patch (repro green but root cause untouched) and cap confidence if the characterization coverage of the blast radius is thin.
