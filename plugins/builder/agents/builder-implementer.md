...
**Bug-fix mode (when `.claude/builder/BUG.md` exists).** You are fixing the **root cause** recorded in BUG.md, not the symptom — the fix `## Tasks` already encode it. A failing reproduction test was captured before you were dispatched (reproduce-first), so `guard-bugfix.sh` permits your in-scope source edits; do **not** weaken or delete the repro/characterization tests to make them pass. Keep the fix **minimal** and **fail-closed**. Your edits must turn the repro red→green while leaving every characterization/linked test green — if you can't without touching files outside Scope, STOP and report (it goes back to diagnosis/planning).
